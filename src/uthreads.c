...
struct user_thread_st {
    thread_func         func;
    uint8_t volatile    status;                     //volatile since could be set from the signal handler
    sigjmp_buf          environ;
    void*               params;
    mutex_id            mutex_waited;
};

struct user_thread_st uthreads[num_user_threads];   //TODO could be implemented as a linked list for greater flexibility

enum thread_ids {
    THREAD_NONE=255
};

enum user_thread_status {   //used in the longjmp from thread to scheduler
    UTS_NOT_RUN     =1,
    UTS_DEAD        =2,
    UTS_PREEMPTED   =3,
    UTS_LOCK_WAIT   =4
...
enum scheduler_status {     //used in the longjmp from scheduler to preempted thread
    SS_CONTINUE     =1,
    SS_LOCK_SECURED =2
};
...
uint8_t ut_current;
sig_atomic_t volatile ut_quantum_finished;   //volatile because set in signal handler


struct mutex_st {
    mutex_owner     owner;          //OWNER_NONE if free mutex, else uthread index
    uint8_t         count;          //recursive mutex
};
...
enum interrupt_parms {
    signal_interrupt    = SIGALRM,
    itimer_mode         = ITIMER_REAL
};
...
void signal_schedule(void){                     //schedule a single SIGVTALRM in ut_quantum ms
    ...
    setitimer(itimer_mode, &itv, NULL);
}

void signal_cancel(void){                       //cancel any pending SIGVTALRM
...
void mutex_lock(mutex_id id){
    struct mutex_st * const m=&mutexes[id];     //shortcut
...
bool uthread_yield(bool forced){                            //returns true if it actually yielded
    bool result=false;
    if(forced || ut_quantum_finished){
        if(CONFIG_VERBOSE) printf("YIELDING in thread %u\n",ut_current);
        if(forced){
            signal_cancel();
        }
        if(!sigsetjmp(uthreads[ut_current].environ, true)){
        ...
        result=true;
        if(CONFIG_VERBOSE) printf("RETURNED to thread %u\n",ut_current);
    }
    return result;
}

void timer_catcher(int signum){ 
...
bool mythread_yield(bool forced){
    if(CONFIG_PREEMPTIVE_SCHEDULING && !forced) return false;
...
int mythread_kill(int tid){
    ...
    uthreads[tid].status=UTS_DEAD;                                  //TODO: would be good to clean up the killed thread's locks
    return 0;
...
void ut_scheduler_watchdog_reset(void){    //take note that the scheduler still has something to run
    ut_visited=0;
    uthreads_locked_count=0;
    if(CONFIG_VERBOSE) printf("WATCHDOG RESET by thread %u\n", ut_current);
}

uint8_t uthread_scheduler(void){                        //returns how many threads were locked at the end
    //init auxiliary vars
    ...
    if(CONFIG_VERBOSE) printf("SCHEDULER, will try now thread %u\n",ut_current);
    
    //find next runnable thread, or return if none
    
    while ((uthreads[ut_current].status==UTS_DEAD) && !(ut_visited==num_user_threads)) {
        ut_visited++;
        if(CONFIG_VERBOSE) printf("current=%u, DEAD, visited=%u\n",ut_current,ut_visited);
        ut_current++;
    ...
    switch (uthreads[ut_current].status) {
        ...
        case UTS_NOT_RUN:{
            ...
            if(CONFIG_VERBOSE) printf("ENDED thread %u\n",ut_current);
            mythread_exit();
            break;
        }
        case UTS_LOCK_WAIT:{
            ...
            if(mutex_is_free(m_waited)){
                ...
                if(CONFIG_VERBOSE) printf("THREAD %u LOCKED, count=%u, visited=%u\n",ut_current, uthreads_locked_count, ut_visited);
                siglongjmp(scheduler_environ,UTS_LOCK_WAIT);                                      
            }
            break;
        }
